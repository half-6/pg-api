...
function buildSelect(table,jsonQuery) {
    ...
    let countSQL = [];
    let parameters = {};
    let limit = (jsonQuery[$LIMIT] && jsonQuery[$LIMIT]<= table.settings.max_limit && jsonQuery[$LIMIT] >0)?jsonQuery[$LIMIT]:table.settings.limit;
    ...
    let selectQuery = sql.join(" ");
    let countQuery = countSQL.join(" ");
...
function build(action,table,jsonQuery) {
    switch (action.toLowerCase()){
        ...
        case "upsert": return buildUpsert(table,jsonQuery);
        default:
