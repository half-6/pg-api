...
module.exports = meta =>{
    ...
    function parseQuery(query) {
        ...
        else
        {
            ...
            {
                let queryValue = queryValueParse(query[key]);
                if(key.startsWith("$"))
                {
    ...
    function hasCompositeAccess(compositeName) {
        return !(meta && meta.composites && meta.composites[compositeName] == false);
    }
    function hasEnumAccess(enumName) {
        return !(meta && meta.enums && meta.enums[enumName] == false);
    }
    function build() {
        $router.get("/composite/:compositeName",(req,res,next)=>{
            $pgHelper.getSchema()
                .then(function (result) {
                    let composites = result.composites;
                    if(composites[req.params.compositeName] && hasCompositeAccess(req.params.compositeName))
                    {
                        buildSuccess(res)(composites[req.params.compositeName])
                    }
                    else
                    {
                        throw new Error(`Specific composite ${req.params.compositeName} not found`);
                    }
                }).catch(next);
        },nextHandler);
        $router.get("/enum/:enumName",(req,res,next)=>{
            $pgHelper.getSchema()
                .then(function (result) {
                    let enums = result.enums;
                    if(enums[req.params.enumName] && hasEnumAccess(req.params.enumName))
                    {
        ...
        $router.all("/:table",(req,res,next)=>{
            ...
            {
                let customAction = meta.custom[req.params.table];
                if(customAction.method.map(c=>{return c.toLowerCase()}).indexOf(req.method.toLowerCase())<0)
                {
                    throw new Error("Specific HTTP Method does not support yet");
                }
                let jsonObj = Object.keys(req.query).length > 0?req.query:req.body;
                action = $pgHelper.custom(req.params.table, parseQueryNoWhere(jsonObj));
            }
            else
            {
                switch (req.method) {
                    case "GET":
                        action = $pgHelper.select(req.params.table, parseQuery(req.query));
                        break;
                    case "DELETE":
                        action = $pgHelper.delete(req.params.table,parseQueryNoWhere(req.query));
                        break;
                    case "PATCH":
        ...
        $router.all("/:table/:id",(req,res,next)=>{
            ...
            switch (req.method) {
                ...
                case "PUT":
                    if(!req.body) throw new Error("Invalid json query");
                    action = $pgHelper.upsert(req.params.table,req.body, req.params.id);
                    break;
                default:
