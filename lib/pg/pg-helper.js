...
module.exports = function (meta) {
    ...
    function checkTableAccess(tableName,tableAction) {
        if(meta.tables && meta.tables[tableName])
        {
            let actions = meta.tables[tableName];
            if(actions[tableAction]==false)
            {
    ...
    async function select(tableName,jsonQuery){
        ...
        checkTableAccess(tableName,tableAction.select);
        const builder = $json2Sql.buildSelect(table, jsonQuery);
        before({action:"select",table,builder,jsonQuery});
        ...
        after({action:"select",table,builder,jsonQuery},output);
        return output;
    ...
    async function update(tableName,jsonQuery){
        const table = await getTable(tableName);
        checkTableAccess(tableName,tableAction.update);
        const builder = $json2Sql.buildUpdate(table, jsonQuery);
        before({action:"update",table,builder,jsonQuery});
        let result = await $db.conn.result(builder.SQL, builder.parameters);
        $logger.info(`Updated ${result.rowCount} rows from ${tableName}`);
        after({action:"update",table,builder,jsonQuery},result);
        return result.rowCount;
    }
    //endregion
    ...
    async function insert(tableName,jsonQuery){
        const table = await getTable(tableName);
        checkTableAccess(tableName,tableAction.insert);
        const builder = $json2Sql.buildInsert(table, jsonQuery);
        before({action:"insert",table,builder,jsonQuery});
        let result = await $db.conn.query(builder.SQL, builder.parameters);
        $logger.info(`Insert ${result.length} rows from ${tableName}`);
        after({action:"insert",table,builder,jsonQuery},result);
        return result;
    }
    //endregion
