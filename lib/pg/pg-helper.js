...
module.exports = function(meta) {
  ...
  async function func(functionName, jsonQuery, queryResult) {
    ...
    onRequest({ action, tableName: functionName, jsonQuery });
    const table = await getFunc(functionName);
    checkFuncAccess(functionName, action);
    onQuery({ action, tableName: functionName, table, jsonQuery });
    return $db.conn.func(
      functionName,
    ...
    if (typeof jsonQuery === 'object') {
      if (Array.isArray(jsonQuery)) {
        ...
        for (const param in params) {
          jsonQuery[param] && values.push(jsonQuery[param]);
        }
        return values;
      }
    }
